...
export default function OrderPage() {
  ...
  // Data dummy order (nanti dari localStorage/sessionStorage/Supabase)
  const MOCK_ORDER = {
    orderId: orderCode || 'RITW-123456',
    event: {
    ...
    payment: {
      amount: 350000,
      status: 'pending',
      expiresAt: new Date(Date.now() + 5 * 60 * 1000).toISOString(), // 5 menit dari sekarang
      qrisImage: '/images/payment-qris.jpeg', // Pastikan file ini ada di public/images/qris/
    }
  }
  ...
  const handleConfirmPayment = () => {
    ...
    const message = `Halo Run in the Wood! Saya sudah melakukan pembayaran untuk order ${orderData.orderId}.

Data Peserta:
- Nama: ${orderData.participant.fullName}
- Email: ${orderData.participant.email}
- No. HP: ${orderData.participant.phoneNumber}
- Event: ${orderData.event.title}
- Tanggal: ${formatDate(orderData.event.date)}
- Jumlah: ${formatPrice(orderData.payment.amount)}
...
Mohon konfirmasi pembayaran saya. Terima kasih!`
  ...
  if (isLoading) {
    return (
      <main className="min-h-screen bg-gradient-to-b from-forest-950 to-forest-900">
        <Header />
        <div className="container mx-auto px-4 py-20">
          <div className="max-w-3xl mx-auto text-center">
            <Iconify icon="svg-spinners:ring-resize" className="h-12 w-12 mx-auto mb-6 text-trail-500" />
            <p className="text-forest-300">Loading order details...</p>
          </div>
        </div>
      </main>
    )
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 rounded-full border border-trail-500/30 bg-trail-500/10 px-4 py-2 mb-4">
              ...
              <span className="text-sm text-trail-400">Order #{orderData.orderId}</span>
            </div>
          ...
          <div className="grid lg:grid-cols-2 gap-8">
              ...
              <Card className="text-center p-6">
                ...
                <div className="relative max-w-xs mx-auto mb-6">
                  <div className="aspect-square bg-white rounded-3xl ">
                    {orderData.payment.qrisImage ? (
                      <Image
                        src={orderData.payment.qrisImage}
            ...
            <div className="space-y-6">
                  ...
                  <div className="pb-4  border-forest-800">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div>
                        ...
                        <p className="font-medium">{orderData.participant.fullName}</p>
                      </div>
                      <div>
                        <p className="text-forest-400">Gender</p>
                        <p className="font-medium capitalize">{orderData.participant.gender}</p>
                      </div>
                      <div>
                        <p className="text-forest-400">Email</p>
                        <p className="font-medium">{orderData.participant.email}</p>
                      </div>
                      <div>
                        <p className="text-forest-400">Phone</p>
                        <p className="font-medium">{orderData.participant.phoneNumber}</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-3">
                <a
                  href={`https://wa.me/6287737815286?text=Halo%20Run%20in%20the%20Wood,%20saya%20membutuhkan%20bantuan%20untuk%20order%20${orderData.orderId}`}
                  target="_blank"
